...
public class PermissionService {
    ...
    public boolean hasPermission(long attachmentId) {
        ...
        if (!contentPermissionManager.hasContentLevelPermission(user, "Edit", attachment.getContainer())) {
        ...
        Map<String, Long> users = this.spacePermissionManager.getUsersForPermissionType("REMOVEATTACHMENT", space);
        if (users.get(user.getName()) != null) {
            log.info("user has user permission");
            return true;
        }
        Map<String, Long> groups = spacePermissionManager.getGroupsForPermissionType("REMOVEATTACHMENT", space);
        ...
        log.warn("seems user {} has no permission to delete this attachment", user.getName());
        return false;
    }
