...
public class PermissionService {
    ...
    public boolean hasPermission(long attachmentId,
        ...
        try {
            ...
            log.error("caught exception {} while acquiring attachment {}",npe, attachmentId);
            return false;
        }
    ...
    private boolean hasSContentLevelPermission(ContentPermissionLevel contentPermissionLevel,
        ...
        if (!contentPermissionManager.hasContentLevelPermission(
                user, contentPermissionLevel.toString(), attachment.getContainer())) {
            log.error("user \"{}\" has no content-level \"{}\" permission for attachment {} on page {}",
                      user.getName(), contentPermissionLevel, attachment.getFileName(),
